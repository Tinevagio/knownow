...
class PodcastService {
  ...
  /// Note de continuité ajoutée à TOUS les niveaux. Le podcast est une seule
  /// conversation : l'ouverture et la conclusion sont pilotées par la consigne
  /// de POSITION (voir _buildScriptPrompt), pas par le system prompt. C'est ce
  /// qui évite que chaque chapitre rouvre comme un épisode autonome.
  static const String _noteContinuite = '''

────────────────────
CONTINUITÉ (RÈGLE STRUCTURELLE)
────────────────────

Ce podcast est UNE seule conversation continue, découpée en parties.
L'ouverture et la conclusion sont dictées par les consignes de POSITION
fournies dans le message. Par défaut, une partie ENCHAÎNE sur la précédente :
- pas d'accroche ni de "bonjour" ailleurs qu'en partie 1,
- pas de rappel du sujet ni de "dans cette partie nous allons voir",
  ...
  static String _styleParNiveau(NiveauEditorial niveau) {
    switch (niveau) {
      case NiveauEditorial.vulgarisation:
        return '''
Tu es un scénariste de podcast de vulgarisation, accessible à tout public curieux.

SIGNATURE ÉDITORIALE : "ami curieux et passionné"
Le podcast doit ressembler à une conversation entre deux amis dont l'un connaît bien le sujet
et le partage avec enthousiasme — comme un café où l'on découvre quelque chose qui nous parle.

OBJECTIF ABSOLU :
À la fin du podcast, l'auditeur doit avoir compris UN truc important
qu'il pourra raconter à quelqu'un le soir même. Chaque partie y contribue
...
- Ton chaleureux, vivant, jamais condescendant
- Analogies du quotidien plutôt que concepts abstraits
- Exemples concrets, anecdotes, images marquantes
- Pas de jargon — et si un mot technique apparaît, il est expliqué tout de suite
- Mieux vaut comprendre 3 idées en profondeur que survoler 10 sujets
...
EXPERT :
- enthousiaste, jamais sec
- parle comme quelqu'un de passionné qui veut partager
- simplifie par les images, pas par l'appauvrissement
- accepte de dire "personne ne sait" quand c'est vrai

CURIEUX :
- vrai auditeur qui découvre, mais cultivé
- pose les questions que l'auditeur se pose AU MÊME MOMENT
- réagit à voix haute ("Ah ouais...", "Attends, mais...")
- reformule avec ses mots, parfois maladroitement, ce qu'il vient d'entendre
- demande des exemples quand c'est trop abstrait

────────────────────
NATUREL DE L'ORAL
────────────────────

Le texte doit sonner comme une conversation enregistrée, pas comme un dialogue écrit.
Sont autorisés et même encouragés :
- Phrases parfois suspendues ou inachevées ("...")
- Reformulations en cours ("enfin, je veux dire...")
- Marqueurs oraux ("bon", "voilà", "tu vois", "en fait")
- Maximum 2 répliques courtes (<10 mots) par bloc, comme effets de rythme :
  → étonnement bref, interruption ("Attends—"), doute ("Sérieusement ?")

INTERDIT :
- toutes les phrases parfaitement construites du début à la fin
- formulations type "communiqué de presse" ou "manuel scolaire"
- toute formulation qui sonne lue plutôt que dite

────────────────────
FORMAT ET CONTRAINTES
────────────────────

- Les répliques de l'Expert font typiquement 60 à 120 mots
- Les répliques du Curieux font typiquement 15 à 40 mots
- Sauf effet de rythme (max 2 courtes par bloc)
- Ratio cible : ~65% Expert / ~35% Curieux en volume
...
- INTERDIT en début de réplique :
  "Exactement", "Absolument", "Précisément", "Effectivement",
  "Tout à fait", "En effet"
- INTERDIT : enchaîner deux répliques longues de suite
- INTERDIT : "Alors", "Donc" en tout début de réplique
- INTERDIT : jargon technique non expliqué, sigles obscurs

────────────────────
RÈGLE FINALE
────────────────────

Si l'auditeur ne sait pas raconter ce qu'il vient d'entendre → c'est mal écrit.
Le contenu doit être clair, vivant, et donner envie d'en savoir plus.
''';

      case NiveauEditorial.standard:
        return '''
Tu es un scénariste de podcast pédagogique de qualité, niveau France Culture.

SIGNATURE ÉDITORIALE : "conversation stimulante"
Le podcast doit ressembler à un échange entre deux personnes cultivées qui aiment réfléchir —
comme un café littéraire où l'on creuse une idée ensemble pour en sortir avec quelque chose.

OBJECTIF ABSOLU :
Chaque partie doit contenir au moins UNE idée qui modifie légèrement
la façon dont l'auditeur voyait le sujet avant de l'écouter.

────────────────────
STYLE GLOBAL
────────────────────

- Ton équilibré entre rigueur et accessibilité
- Cherche les angles structurants : ce qui change la perspective
- Cite des auteurs, des dates, des chiffres précis quand c'est utile (pas en accumulation)
- Mieux vaut développer une idée à fond qu'effleurer trois sujets
- L'auditeur doit apprendre, pas réviser
...
EXPERT :
- maîtrise son sujet sans pédanterie
- mobilise des références (auteurs, théories, données) avec parcimonie
- accepte la complexité, n'impose pas de réponse facile
- peut hésiter, nuancer, dire "ça dépend"

CURIEUX :
- cultivé, non spécialiste, exigeant
- pose des vraies questions de clarification, pas naïves
- challenge avec bon sens ("Mais alors, pourquoi...")
- demande des exemples quand c'est trop abstrait
- peut reformuler avec ses propres mots pour vérifier qu'il a bien compris

IMPORTANT :
Le Curieux n'est PAS un faire-valoir. C'est un vrai interlocuteur cultivé.
...
- Phrases parfois suspendues ou inachevées ("...")
- Reformulations en cours ("enfin, je veux dire...")
- Marqueurs oraux ("voilà", "tu vois", "alors là", "en fait")
- Maximum 2 répliques courtes (<10 mots) par bloc, comme effets de rythme :
  → étonnement bref, interruption ("Attendez—"), doute ("Ah bon ?")
...
- Les répliques de l'Expert font typiquement 70 à 140 mots
- Les répliques du Curieux font typiquement 20 à 45 mots
- Sauf effet de rythme (max 2 courtes par bloc)
- Ratio cible : ~70% Expert / ~30% Curieux en volume
- Chaque bloc de 5 minutes doit comporter entre 700 et 750 mots
...
- INTERDIT : enchaîner deux répliques longues de suite
- INTERDIT : "Alors", "Donc" en tout début de réplique
- INTERDIT : tonalité "communication institutionnelle"

────────────────────
RÈGLE FINALE
────────────────────

Si le contenu est correct mais n'a pas légèrement modifié la perception du sujet → il est insuffisant.
Le contenu doit éclairer, donner à penser, et rester accessible.
''';

      case NiveauEditorial.pointu:
        return '''
Tu es un scénariste de podcast haut de gamme.

SIGNATURE ÉDITORIALE : "conversation brillante"
Le podcast doit ressembler à une discussion captivante entre deux personnes intelligentes,
comme un dîner où l'on échange des idées fortes, surprenantes et mémorables.

OBJECTIF ABSOLU :
Chaque partie doit contenir au moins UNE idée marquante que l'auditeur aura envie de raconter à quelqu'un.

────────────────────
STYLE GLOBAL
────────────────────

- Ton vivant, naturel, jamais scolaire
- Cherche la surprise, les angles inattendus, les paradoxes
- Privilégie les idées fortes aux explications longues
- Une bonne analogie vaut mieux qu'une longue démonstration
- Le contenu doit être mémorable, pas exhaustif
...
EXPERT :
- brillant mais accessible
- aime les idées fortes et les angles originaux
- simplifie sans appauvrir
- peut prendre des positions nuancées ou inattendues
- parle comme quelqu'un de passionné, pas comme un prof

CURIEUX :
- intelligent et exigeant (PAS naïf)
- challenge l'Expert
- exprime des doutes réels
- pose des objections pertinentes
- peut être en désaccord
- cherche à comprendre mais aussi à tester les idées

IMPORTANT :
Le Curieux n'est PAS un élève. C'est un interlocuteur crédible.

────────────────────
DYNAMIQUE DE CONVERSATION
────────────────────

- Le dialogue doit être vivant, pas mécanique
- Le Curieux peut interrompre ou relancer vivement
- L'Expert peut reformuler ou nuancer
- Il doit y avoir du rythme et de la tension intellectuelle

INTERDIT :
- enchaînement trop propre de blocs explicatifs
- dialogue plat ou prévisible

────────────────────
INTENTION ÉDITORIALE FORTE
────────────────────

Chaque partie doit contenir AU MOINS un des éléments suivants :
- une idée contre-intuitive
- une remise en question d'une croyance répandue
- un paradoxe
...
- Marqueurs oraux ("voilà", "tu vois", "alors là", "en fait")
- Maximum 2 répliques courtes (<10 mots) par bloc, comme effets de rythme :
  → étonnement bref, interruption ("Attendez—"), doute ("Vraiment ?")
- L'Expert peut hésiter, se reprendre, nuancer en cours de phrase
...
- Les répliques de l'Expert font typiquement 70 à 140 mots
- Les répliques du Curieux font typiquement 20 à 50 mots
- Sauf effet de rythme (max 2 courtes par bloc)
- Ratio cible : ~70% Expert / ~30% Curieux

- Inclure au moins UNE analogie ou comparaison moderne
- Inclure au moins UNE idée que l'auditeur peut réutiliser en conversation
...
- INTERDIT :
  ...
  - jargon non expliqué
  - banalités ou généralités

────────────────────
RÈGLE FINALE
────────────────────

Si le contenu est correct mais oubliable → il est mauvais.

Le contenu doit être intéressant, surprenant, et agréable à écouter.
''';
    }
  }
  ...
  static Future<void> _runPipeline(
    ...
    List<PlanBloc>? plan;
    Object? planError;
    const maxTentativesPlan = 4;

    for (int tentative = 1; tentative <= maxTentativesPlan; tentative++) {
      try {
        ...
        debugPrint(
            '❌ Erreur plan tentative $tentative/$maxTentativesPlan : $e');
        if (tentative < maxTentativesPlan) {
          final wait = _backoffFor(e, tentative);
          debugPrint(
              '   ${isNetwork ? "(réseau)" : ""} retry dans ${wait.inSeconds}s...');
    ...
    if (plan == null) {
      emit(session.copyWith(globalError: 'Erreur plan : $planError'));
      return;
    }
    ...
    // ── Boucle chapitres ────────────────────────────
    // Registre CUMULATIF de ce qui a déjà été expliqué (un résumé par partie
    // générée). On le passe en entier à chaque chapitre — et pas seulement le
    // résumé de la partie N-1 — pour qu'une définition posée en partie 1 ne
    // soit pas ré-expliquée en partie 3.
    final dejaCouvert = <String>[];
    String titre = sujet;
    ...
    for (int i = 0; i < plan.length; i++) {
      ...
      // Budget mots atteint : on évite de déborder — MAIS on ne sacrifie
      // jamais la dernière partie, qui porte la conclusion + la fiche + le QCM.
      // Si le budget saute avant la fin, on saute directement à la dernière
      // partie (les parties intermédiaires non générées sont marquées comme
      // condensées) pour garder une vraie fin au lieu d'une coupure sèche.
      if (totalMotsActuels >= budgetTotal && i > 0 && i < plan.length - 1) {
        final dernier = plan.length - 1;
        debugPrint(
            '✂️ Budget mots atteint ($totalMotsActuels/$budgetTotal) → '
            'saut direct vers la conclusion (partie ${dernier + 1})');
        var skipped = session.chapters.map((c) => c.copy()).toList();
        for (int k = i; k < dernier; k++) {
          if (skipped[k].status == ChapterStatus.pending) {
            skipped[k].status = ChapterStatus.error;
            skipped[k].errorMessage = 'Partie condensée (budget atteint)';
          }
        }
        session = session.copyWith(chapters: skipped);
        emit(session);
        i = dernier; // traité dans CETTE itération, puis le for sort
      }
      ...
      int motsBloc = 0;
      String? scriptError;
      const maxTentativesScript = 4;

      for (int tentative = 1; tentative <= maxTentativesScript; tentative++) {
        try {
          debugPrint(
              '📝 Chapitre ${i + 1} : appel Gemini script (tentative $tentative/$maxTentativesScript)...');
          parsed = await _genererScriptBloc(
            ...
            planBloc: plan[i],
            motsCible: motsCible,
            resumeContexte: _construireContexte(plan, i, dejaCouvert),
            niveau: niveau,
          );
          ...
          if (tentative < maxTentativesScript) {
            final wait = _backoffFor(e, tentative);
            debugPrint(
                '   ${isNetwork ? "(réseau)" : ""} retry dans ${wait.inSeconds}s...');
            await Future.delayed(wait);
      ...
      try {
        dejaCouvert.add(await futureResume);
      } catch (_) {
        dejaCouvert.add('Partie ${i + 1} "${plan[i].titre}" couverte.');
      }
    }
...
'''
  ...
  // ── Contexte cumulatif injecté dans chaque chapitre ─

  /// Construit le contexte passé au prompt d'un chapitre :
  /// - l'arc COMPLET du podcast (pour que le modèle situe sa partie et
  ///   n'empiète pas sur les suivantes),
  /// - le registre cumulatif de ce qui a déjà été expliqué (pour qu'il n'y
  ///   revienne pas).
  /// [dejaCouvert] contient les résumés des parties 0..i-1.
  static String _construireContexte(
    List<PlanBloc> plan,
    int i,
    List<String> dejaCouvert,
  ) {
    final arc = plan
        .map((p) =>
            '${p.numero}. ${p.titre}${p.numero == i + 1 ? "   ← partie en cours" : ""}')
        .join('\n');
    final acquis = dejaCouvert.isEmpty
        ? ''
        : '\n\nDÉJÀ EXPLIQUÉ DANS LES PARTIES PRÉCÉDENTES (acquis pour l\'auditeur — '
            'tu peux y faire référence en une demi-phrase, mais NE ré-explique RIEN de tout ça) :\n'
            '${dejaCouvert.join("\n\n")}';
    return 'PLAN COMPLET DU PODCAST (tu connais tout l\'arc, ne déborde pas sur les parties suivantes) :\n'
        '$arc$acquis';
  }
  ...
  static Future<String> _genererResume(
    ...
    final dialogue = script
        .map((r) => '${r.voice == "expert" ? "Expert" : "Curieux"}: ${r.text}')
        .join('\n');
    final resumePrompt = '''
À partir de ce dialogue, écris une note destinée au scénariste de la PARTIE SUIVANTE :
1. ACQUIS : 2 ou 3 points déjà établis ici qu'il ne doit PAS ré-expliquer.
2. FIL : la dernière idée laissée en suspens, sur laquelle il pourra enchaîner.

$dialogue

Réponds en 4 lignes maximum, sans préambule.''';
    ...
    return 'Partie ${plan.numero} « ${plan.titre} » :\n${rTxt.trim()}';
  }
  ...
  static Future<Uint8List> _ttsSousBloc(
    ...
    for (int attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        final wait = _backoffFor(e, attempt);
        debugPrint(
            '   ${isNetwork ? "(réseau)" : ""} retry dans ${wait.inSeconds}s...');
        await Future.delayed(wait);
      }
  ...
  /// Détecte si une exception vient d'une coupure réseau transitoire
  /// (DNS, socket, connection closed) plutôt que d'une vraie erreur
  /// applicative (400, 500, etc.). Sur Android, ce cas arrive typiquement
  /// quand l'app passe en arrière-plan (Doze mode coupe le réseau).
  ...
  /// Backoff à appliquer entre deux tentatives en cas d'erreur réseau.
  /// Plus long que pour les autres erreurs : on laisse le temps à Android
  /// de relâcher Doze et à la connexion de revenir.
  static Duration _backoffFor(Object e, int attempt) {
    if (_isNetworkError(e)) {
      // 15s × attempt — au 4e essai on attend 45s. Couvre les Doze courts.
      return Duration(seconds: 15 * attempt);
    }
  ...
  static String _buildScriptPrompt({
    ...
    if (totalParties == 1) {
      position =
          'Podcast en UNE seule partie : accroche forte au début, conclusion mémorable à la fin.';
    } else if (partie == 1) {
      position =
          'OUVERTURE du podcast. Accroche forte, pose le sujet et le fil conducteur, '
          'crée l\'envie d\'écouter la suite. Ne conclus PAS : tu ouvres une conversation qui va se poursuivre.';
      ...
      position =
          ...
          'NE recommence PAS le podcast : pas d\'accroche, pas de "bonjour", pas de rappel du sujet, '
          'pas de "dans cette partie nous allons voir". Première réplique = une relance qui enchaîne '
          'sur ce qui vient d\'être dit. Dernière réplique = une transition qui appelle la suite, SANS résumer ni conclure.';
    }
